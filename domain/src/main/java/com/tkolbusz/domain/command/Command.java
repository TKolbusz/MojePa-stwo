...
public abstract class Command<Result, Params> {
    ...
    public Observable<Result> apply(Params params) {
        Observable<Result> resultObservable = buildObservable(params)
                .subscribeOn(threadExecutor.getScheduler())
                .observeOn(postExecutionThread.getScheduler());
        if (debug)
            return resultObservable.compose(debugInfo());
        else
            return resultObservable;
    }

    private ObservableTransformer<Result, Result> debugInfo() {
        return upstream -> upstream.doOnSubscribe(__ -> startTime = System.currentTimeMillis())
                .doOnTerminate(() -> {
